...
TIMING_GLOBS=('**/scene/perf.test.ts' '**/scene/SceneView.test.tsx')
if [ "$LOAD" != "0" ]; then
  ...
  if ( cd "$W" && npx vitest run --maxWorkers=1 scene/perf.test.ts scene/SceneView.test.tsx >/tmp/g-$H-timing.log 2>&1 ); then
