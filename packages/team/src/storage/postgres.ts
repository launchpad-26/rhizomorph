import type { AppliedMigration, EventQuery, EventRow, ProjectionDelta, TeamStorage } from './contract.js'
 ...
 * UPDATE on the three projection tables. `../fold/worker.test.ts` cross-checks
 * the statements this actually sends against that file's own `GRANT`s, so a
 * future `RETURNING` or a read of `events` reddens there rather than on a host.
 *
