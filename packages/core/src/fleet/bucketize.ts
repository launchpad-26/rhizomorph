 ...
 *
 * Deliberately duplicated from `packages/web/src/spark/bucketize.ts` rather
 * than imported: that module sits outside this file's fence (issue #246), and
 * `web/src/panels/ledger/sparkline.ts` still depends on the original in
 * place. Same pure function, no shared home yet — a follow-up should give it
 * one rather than leave two copies drifting.
 */
