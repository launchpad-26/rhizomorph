...
describe('the dispatch, and the check that it took', () => {
  it('raises the page\'s own keydown, with the key the page binds', () => {
    expect(demoDispatchScript('fleet20')).toContain("new KeyboardEvent('keydown'")
    expect(demoDispatchScript('fleet20')).toContain('"2"')
    expect(demoDispatchScript('pathology')).toContain('"3"')
    expect(demoDispatchScript('live')).toContain('"1"')
  })

  it('raises it with no modifier — `useFixtureKeys` ignores anything with one held', () => {
    for (const source of ['live', 'fleet20', 'pathology'] as DemoSource[]) {
      expect(demoDispatchScript(source)).not.toMatch(/altKey|ctrlKey|metaKey/)
    }
  })

  it('expects a simulated log for the fixtures and a live one for live', () => {
    expect(expectSimulated('fleet20')).toBe(true)
    expect(expectSimulated('pathology')).toBe(true)
    expect(expectSimulated('live')).toBe(false)
  })

  it('retries for about two seconds — longer than a mount, shorter than a person\'s patience', () => {
    expect(DEMO_ATTEMPTS * DEMO_RETRY_MS).toBe(2_000)
  })
})

/**
 * The check reads the page for a word the page only shows while a fixture is
 * driving. That word is core's, not this file's — and if a fixture's
 * provenance were reworded, the check would silently pass on nothing, which is
 * the failure shape this repo treats as worse than a crash.
 */
describe('the simulated mark is core\'s own word (#565)', () => {
  const fixtures = readFileSync(
    path.join(REPO_ROOT, 'packages', 'core', 'src', 'fleet', 'fixtures.ts'),
    'utf8',
  )

  it('is how every simulated spec\'s provenance begins', () => {
    const provenances = [...fixtures.matchAll(/provenance: '([^']+)'/g)].map((match) => match[1] ?? '')
    expect(provenances.length).toBeGreaterThan(2)
    for (const provenance of provenances) expect(provenance.startsWith(SIMULATED_MARK)).toBe(true)
  })

  it('is what the verification script looks for', () => {
    expect(DEMO_VERIFY_SCRIPT).toContain(SIMULATED_MARK)
    expect(DEMO_VERIFY_SCRIPT).toContain('document.body.textContent')
  })

  it('is not a word the live label carries — the check would be vacuous if it were', () => {
    const stream = readFileSync(path.join(STREAM_CONTEXT), 'utf8')
    expect(stream).toContain('`live · ${url}`')
    expect('live · http://127.0.0.1:4321').not.toContain(SIMULATED_MARK)
  })
})
