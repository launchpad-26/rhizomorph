 ...
 *
 * VERSION 2 (prd14 ruling 6, ADR-0049): a saved comparison is a projection of
 * one measure and nothing else in v1 — `value` is whichever measure happened
 ...
 * `LabRunOutcomeDTO`, the server's wire type — per run `{ verdict, detail?,
 * cost, duration, commits }`, and per artifact the `measure` and the gate's
 * `provenance`. The reopened surface re-derives through `runForMeasure`
 * (`fromExperiment.ts`), the SAME function the live surface uses. A v1
 * artifact is READ, not migrated: v1 → v2 is not total (the measure is
 * unrecoverable from a v1 file), so there is no upcast — both versions parse
 ...
 * refuse them identically, message and all. ADR-0049 keeps these two copies
 * deliberately, on the condition that the agreement laws hold TWO kinds of
 * coverage, not one: throw-site coverage DERIVED FROM SOURCE (which is why the
 * v1/v2 validation below shares one throw site per message via a common
 * helper rather than repeating a message string in two places — two identical
 * literal throw sites would make that coverage ambiguous, see
 * `parser-agreement-law.test.ts`'s own doc, axis B), AND accepted-value
 * coverage — the fixture must exercise every member of every union v2 admits
 * (every `MeasureV2`, every `ComparisonProvenanceV2['source']`), and both the
 * finite and non-finite form of every numeric field. Site coverage is NOT
 * value coverage: narrowing an accepted union removes no throw site and adds
 * no message, so a one-sided narrowing reads as fully covered while the two
 * copies have genuinely diverged — measured on this build, ADR-0049's own
 * Consequences record the exact narrowings that passed both agreement laws
 * before the fixture was widened to catch them.
 */
...
/** The four measures the compare surface can read a run for (prd53 S2's own `Measure` union, restated here rather than imported — this subtree's artifact format keeps its own vocabulary, ADR-0049). */
...
function parseRunV1(record: unknown): Run {
