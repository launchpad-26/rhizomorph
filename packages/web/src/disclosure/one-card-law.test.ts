 ...
 *   ADR-0044 rules that a menu is not card chrome: `DisclosureContent` is
 ...
 * button anywhere escape, which is a far bigger hole than ADR-0044 admits to.
 */
 ...
 * explanation — prd-30 S1's sentence, at last (#221, ADR-0044).
 */
...
describe('S1’s whole sentence: no directory grows its own hover explanation (#221, ADR-0044)', () => {
  ...
  it('does not name a menu — the MarkHoverCard shape, written as a fixture rather than by naming the file', () => {
    // ADR-0044's third clause. Every row is a control, so it is a list of
