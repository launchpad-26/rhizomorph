 ...
 * - `rounded-ctl` — the radius is the control radius by name, not by luck.
 *
...
const BUTTON_BASE =
  'focus-ring min-h-6 rounded-ctl border px-2 py-1 text-inst leading-none normal-case tracking-normal transition-[color,border-color] duration-(--duration-touch) disabled:opacity-50'
...
export const FIELD =
  'focus-ring min-h-6 max-w-full rounded-ctl border border-(--line-hair) bg-(--surface-floor) px-2 py-1 font-sans text-inst normal-case tracking-normal text-(--ink-primary)'
