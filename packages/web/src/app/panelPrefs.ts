...
/**
 * Deliberate product ruling (prd1 UI section): collisions must default to
 * expanded, not collapsed — collision warnings are the day's own failure
 * mode, made visible before merge pain, and must not be hideable by default.
 * Every other panel also defaults expanded (see the `?? false` fallback
 * below); this entry exists so the ruling survives a future panel-density
