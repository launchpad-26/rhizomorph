 ...
 * cut the density band). This file answers "which instants would I point at
 * first" — pure, single-purpose, no view, no clock, `laneOf` imported rather
 * than re-derived (`laneOf.ts` owns lane identity; copying its switch here
 * would be drift-by-construction, the same warning `laneOf.ts`'s own module
 * note makes about `buildFleet`).
 *
 ...
 *   reason to hold that line more carefully, not less.
 ...
 * or re-deriving `buildFleet`'s fold beside it, the exact drift `laneOf.ts`
 * already warns against. No mark kind is emitted for it.
 *
 * Laws, each restated as a test in `chapters.test.ts`:
 *
 ...
 * than a span: `163 landed · 14:32:07`.
 */
