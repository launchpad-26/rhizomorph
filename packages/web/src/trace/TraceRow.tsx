...
export function RowLabel({ node }: RowLabelProps) {
  ...
  return (
    <span className="flex min-w-0 items-baseline gap-2">
      ...
      <span className="min-w-0 truncate font-mono text-inst text-(--ink-body)">{rowName(span)}</span>
      {span.kind === 'llm_request' && span.ttftMs !== null ? (
        <span className="shrink-0 text-inst-dense text-(--ink-dim)">ttft {formatSpan(span.ttftMs)}</span>
      ) : null}
    </span>
  )
}
...
export function RowMeta({ node }: RowMetaProps) {
  ...
  return (
    <span data-testid="trace-duration" className="text-inst-dense text-(--ink-dim)">
