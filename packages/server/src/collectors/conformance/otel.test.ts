...
describe('conformance suite: otel — the check bites (mutation, executed)', () => {
  ...
  it('routes every otel fixture to its own parser — the right one by name, never merely some parser (#460)', async () => {
    ...
    expect(routed).toEqual({
      ...
      'gemini-cli-0.55.1-otlp-3-logs.json': 'logs',
      'gemini-cli-0.55.1-otlp-4-metrics.json': 'metrics',
      'gemini-cli-0.55.1-otlp-5-logs.json': 'logs',
      'gemini-cli-0.55.1-otlp-6-traces.json': 'traces',
      'gemini-cli-0.55.1-otlp-7-logs.json': 'logs',
      'gemini-cli-0.55.1-otlp-8-traces.json': 'traces',
      'gemini-cli-0.55.1-otlp-9-metrics.json': 'metrics',
