...
describe("the README's outbound-fetch recipe names exactly the real call sites, and only those (#23)", () => {
  ...
  const EXPECTED_CALL_SITES: ReadonlyArray<{ file: string; count: number }> = [
    ...
    // prd-14 ruling 5, #214: the comparison save client — the app's seventh
    // mutating call, one aliased fetch (`const impl = fetchImpl ?? globalThis.fetch`).
    { file: path.join('packages', 'web', 'src', 'lab', 'compare', 'save.ts'), count: 1 },
