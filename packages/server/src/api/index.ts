...
export function registerApiRoutes(app: FastifyInstance, ctx: ServerContext): void {
  ...
  // The frame's two closed gaps (prd-55 ruling 6, #402): a lane's OTel
  // readings sliced at a byte, and a lane's footprint through the two named
  // core selectors — see `lab-series.ts`'s own doc for why byte, not
  // wall-clock, and why the intersection is a decoration, not a filter.
  registerLabSeriesRoute(app, ctx)
...
export const ROUTE_CLASSES: readonly RouteClassification[] = [
  ...
  // Gated reads (19) — prd-29 wave 1's keystone (ruling 1 / ADR-0024) plus
  ...
  // #60) at the end of this block, and prd-55 ruling 6's lab transcript,
  // telemetry and footprint reads (#384, #402) beside the laboratory's other
  // reads. Each carries
  ...
  // prd-55 ruling 6 (#402): the frame's telemetry and footprint positions,
  // closing its own two stated gaps — a lane's OTel readings sliced at a
  // byte, and a lane's footprint through core's two named selectors.
  { method: 'GET', url: '/api/lab/telemetry', routeClass: 'gated-read' },
  { method: 'GET', url: '/api/lab/footprint', routeClass: 'gated-read' },
