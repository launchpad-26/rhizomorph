...
YOUR ISSUE — #52 (52. rhizomorph doctor — say what is missing and how to fix it)
...
There is no way to find out why the app is not working. `rhizomorph` with no
flags prints one line (`rhizomorph running at <url>`) and validates nothing:
not that the path exists, not that it is a git repo, not that the web build is
present, not the Node version.

**1. Add `rhizomorph doctor`** — a read-only preflight that tells a stranger
...
unguarded and `bin/rhizomorph.mjs` awaits at top level, so `EADDRINUSE` prints a
...
branch is required. Paste real `rhizomorph doctor` output from (a) a healthy
