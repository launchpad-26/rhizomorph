...
YOUR ISSUE — #51 (51. The documented run command installs a stranger's package (npx rhizomorph))
...
`README.md:19` and `docs/demo.md:12` both say `npx rhizomorph <path>`. But:
...
- `package-lock.json` predates the `bin` field added to `packages/server`, so
  `npm ci` cannot link it (verified: `npx --no-install rhizomorph --help` →
  *could not determine executable to run*; `node_modules/.bin/rhizomorph` absent);
- **`rhizomorph` is a real, unrelated package on the public npm registry**
  (`npm view rhizomorph` → "Beautiful UI for showing tasks running on the
...
3. **Remove every `npx rhizomorph` reference** from README and demo docs.
   Document what actually works — verify each before writing it:
   `npm start`, and `node packages/server/bin/rhizomorph.mjs --help`.
