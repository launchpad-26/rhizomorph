...
Found while fact-checking the docs: `rhizomorph --version` **starts the server**
...
that `node packages/server/bin/rhizomorph.mjs --version` now exits instead of
...
e.g. 'timeout 5 node packages/server/bin/rhizomorph.mjs --version', and
