...
- Session log writer: append JSONL events to `~/.local/share/observatory/<repo-slug>/session-<ts>.jsonl` (slug from repo path basename + short hash); reader for history.
...
- CLI entry `observatory [path]`: boots collectors+server on the target repo (default cwd), prints URL. Port 4321 default, `--port` flag.
