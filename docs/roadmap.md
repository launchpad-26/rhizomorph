...
- **prd6 candidate / standing research question — dispatch-policy
  ...
  the Rhizomorph: sensor array in the balcony, policy in the conductor.
...
- **npm name:** scoped — `@kelliherl/rhizomorph`. Publish is deferred until after prd3; clone+run stays the supported path until then. (Unscoped `rhizomorph` is taken; a scoped name needs no reservation.)
