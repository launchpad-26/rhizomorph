...
stand — is worth stating plainly, because the two differ today. Thirteen reads
are gated: `/api/sessions`, `/api/sessions/:id/events`, `/api/lanes`,
`/api/transcript/:lane`, the laboratory's `/api/lab/checkpoints`,
`/api/lab/experiments`, `/api/lab/estimate`, the four reads that postdated the
route arithmetic — `/api/lane-index`, `/api/lane-index/:handle`,
`/api/session-preview/:sessionId`, `/api/concierge/repos` (prd-29 ruling 7,
#58) — and, as of wave 2a, `/api/meta` and `/api/doctor` (prd-29 ruling 7,
#59). One is still tokenless: `/api/stream` is **ruled to gate and not yet
gated**, deferred to a later wave (issue #60) so that no consumer outside the
SPA breaks mid-milestone. `GET /*` stays tokenless *forever*
...
itself depends on. `/api/stream` is the one read still ruled to gate and not
yet in code, which leaves `GET /*` and `/api/stream` outside the gate until it
lands. Read the table's own `routeClass` values for the state of the boundary,
never this paragraph's tense.
...
prd3 (`docs/prds/done/prd-03-viz-design-study.md`) is the "beautiful instrument" design pass, rebuilt in
fenced waves (issues #75–#86) after a same-day spike round chose **Direction
C — Mycelium Pulse-Network** ([ruling 28](prds/done/prd-03-viz-design-study.md)) over the two other spike
...
**The ladder floor is structural, not remembered ([graft g5](prds/done/prd-03-viz-design-study.md)).** The
...
23](prds/done/prd-03-viz-design-study.md)'s cyber-sigilist register): every pathology/state mark is
...
scene does, [graft g1](prds/done/prd-03-viz-design-study.md) falls out for free: **the table is the
...
kind ([graft g4](prds/done/prd-03-viz-design-study.md)), and color is never the sole carrier of a state
([law 9](prds/done/prd-03-viz-design-study.md)) — FROZEN and WAITING, for instance, are built to differ on
...
lives, and it is built directly against [ruling 32](prds/done/prd-03-viz-design-study.md)'s three adopted
...
- **Panel focus (#85)** — `usePanelFocus` (`packages/web/src/app/panelPrefs.ts`)
  ...
  precedence)" cases.
- **Replay's mode shift (#83)** — per [ruling 16](prds/done/prd-03-viz-design-study.md), replay is a full
...
prd4 (`docs/prds/done/prd-04-human-facing.md`) is the operator-review pass that followed prd3: the
...
prd5 (`docs/prds/done/prd-05-beautiful-application.md`) is the "sleek, well formed, beautiful application"
...
prd6 (`docs/prds/done/prd-06-living-cycle.md`) followed the operator's close-out review of prd5: the
...
prd7 (`docs/prds/done/prd-07-procedural-form.md`) followed the operator's close-out review of prd6: "the
...
prd8 (`docs/prds/done/prd-08-published-software.md`) is the publishing round: turning a private,
...
prd10 (`docs/prds/done/prd-10-gorgeous-round.md`) is a scene-beauty pass on top of prd7's procedural
...
prd11 (`docs/prds/done/prd-11-causal-record.md`), ruled 2026-08-04 toward "the forest" (a future,
...
prd12 (`docs/prds/done/prd-12-laboratory.md`) is the constitutional amendment the README's Trust
...
`<input type=range>`, so replay was navigated blind. prd13 (`docs/prds/done/prd-13-tide.md`)
...
- 2026-07-31 — prd3 (issue #75): **the ladder floor (ALL CLEAR structurally
  ...
  `buildFleet`, not the view** ([graft g5](prds/done/prd-03-viz-design-study.md)). The calm branch of the
