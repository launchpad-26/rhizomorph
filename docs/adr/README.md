...
| [0042](0042-the-server-parses-a-comparison-artifact-with-its-own-copy.md) | The server validates a comparison artifact with its own copy of the parser, not a shared schema | 2026-09-08 | superseded by ADR-0049 |
...
| [0046](0046-the-ingest-journal-is-a-crc-framed-append-log.md) | The ingest journal is a CRC-framed two-line append log — a header carrying seq, byte length and CRC-32, then the JSON — because plain JSONL cannot tell a torn tail from a corrupted interior record, and ruling 4 needs those to be different verdicts | 2026-09-09 | proposed (prd-51 ruling 4, recorded on the prd-51 wave-3 ingest build) |
| [0047](0047-a-control-that-explains-its-own-unavailability-is-aria-disabled.md) | A control whose text explains its own unavailability wears `aria-disabled` with a guard inside the handler, not `disabled` — a disabled button leaves the tab order, so the one thing a keyboard could never reach would be the reason it cannot act | 2026-09-10 | accepted (prd-30 wave 4 grooming ruling, recorded on the build #389) |
| [0048](0048-the-instrument-spawns-the-operators-own-tools-as-an-explicit-act.md) | The instrument may spawn the operator's own tools as an explicit act; it never holds or forwards a credential; a spawned tool's egress is the operator's, declared on the control that invokes it | 2026-09-10 | accepted (prd-55 ruling 1, recorded on the build #412) |
| [0049](0049-the-comparison-shape-stays-in-two-copies-at-version-2.md) | The comparison shape stays in two copies at version 2 — the trigger ADR-0042 set has fired, and the criterion behind it is re-decided rather than obeyed, because the drift risk it stood proxy for is now closed by the shared fixture and the two agreement laws | 2026-09-11 | accepted (prd-14 ruling 6), supersedes 0042 |
| [0050](0050-two-identity-planes.md) | Two identity planes that never fuse: org membership (via a GitHub App, not an OAuth App) for humans, `rzk_` keys for machines — no GitHub token is ever accepted on the ingest route, no key is ever derived from one, the ingest hot path never calls GitHub | 2026-09-08 | accepted, prd-51 ruling 8 as amended 2026-09-08 |
| [0051](0051-the-generated-wiki.md) | A third party generates a public wiki over this repo and any agent may query it over MCP, free and uncredentialed while the repo is public — but the repo keeps its own index as the authority, because a generated page is one public document about this project that none of its laws can redden | 2026-09-14 | accepted (operator ruling, recorded on the docs-audit sweep) |
| [0052](0052-the-observer-reads-the-operators-own-agent-processes.md) | The observer's scope widens from the watched repo to the operator's own agent processes, admitted only on an argv signature match, and it never signals a process — amends [0001](0001-read-only-observer-as-a-constitution.md) | 2026-09-15 | proposed (prd-57 ruling 1) |
...
| [0056](0056-the-fold-runs-in-the-app-process.md) | The fold runs inside the `app` container, woken by the ingest `onBatch` seam, drained at boot and re-ticked — rather than as a second compose service, which the unlocked fold cursor makes a corruption rather than a scaling knob | 2026-09-16 | accepted (prd-51 wave 11, #564) |
...
| [0058](0058-retention-is-a-partition-property.md) | Retention drops a whole monthly partition, and only when every project sharing it is past its own ceiling — because `events` is partitioned by `ts` alone and a per-project DELETE is the two-lock path ruling 10 measured away; the cost is that a project with no ceiling can lose rows to another's | 2026-09-17 | accepted (prd-51 wave 12, #559) |
| [0059](0059-the-team-viewers-write-is-defended-by-samesite-and-origin.md) | The team viewer's first browser-reached write is defended by `SameSite=Lax` plus a server-side origin check under the existing membership gate — not by ADR-0008's mutation guard, whose loopback assertions would refuse every real request to a server bound behind Caddy | 2026-09-17 | accepted (prd-51 wave 12, #560) |
